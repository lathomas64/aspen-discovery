...
$cronLogEntry->notes = "Running Talpa groupedWorks cron for settings ".$token;
$cronLogEntry->update();
...
$results = $aspen_db->query('SELECT COUNT(1) AS total
	...
	if ($results) {
		while ($result = $results->fetch()) {
			$cronLogEntry->notes .= '<br/>found '. $result['total'] . ' permanent IDs to send to Talpa for processing';
			$cronLogEntry->update();
		}
	}
...
if ($results) {
	while ($result = $results->fetch()) {
		...
		if( count($permanent_ids) > $BATCH_SIZE ) {
			...
			$cronLogEntry->notes .= '<br/> getting works for batch '. $batchN. ' of size:'. $BATCH_SIZE;
			$cronLogEntry->update();
			...
			foreach ($chunks as $chunk) {
				...
				$cronLogEntry->notes .= "<br/>" . $resA['message'];
				$cronLogEntry->update();
				...
				if($resA['status']=='ok') {
					foreach ($chunk as $permanent_id => $data) {
						...
						$talpaData = null;
						$cronLogEntry->notes .= '<br/>Successfully sent ' . count($chunk) . ' records to Talpa Search for processing.';
						$cronLogEntry->update();
					}
				}else {
					$cronLogEntry->notes .= '<br/>Talpa Search encountered an error while attempting to parse your records.';
					$cronLogEntry->update();
					}
			} // foreach chunksA
...
$cronLogEntry->notes .= "<br/>seenN: ".$seenN . " inserted: ".$insertedN . "updated: ".$updatedN;
$cronLogEntry->endTime = time();
$cronLogEntry->update();
